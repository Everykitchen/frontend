...
const ReservationDetail = () => {
    ...
    const handleCopyAddress = () => {
        if (reservation?.kitchenLocation) {
            navigator.clipboard.writeText(reservation.kitchenLocation)
                .then(() => {
    ...
    if (loading) return <Container><Sidebar activeMenu="예약 내역" /><ContentWrapper>로딩 중...</ContentWrapper></Container>;
    if (error) return <Container><Sidebar activeMenu="예약 내역" /><ContentWrapper style={{color:'red'}}>{error}</ContentWrapper></Container>;
    ...
    return (
        <Container>
            ...
            <ContentWrapper>
                ...
                <DetailCard>
                    <ReservationStatus status={reservation.status}>{getStatusText(reservation.status)}</ReservationStatus>
                    <TopSection>
                        <KitchenImage src={reservation.kitchenImageUrl} alt="주방 이미지" />
                        <KitchenInfo>
                ...
                {showMap && (
                    <Modal>
                        <ModalContent>
                            <ModalHeader>
                                <ModalTitle>{reservation.kitchenName} 위치</ModalTitle>
