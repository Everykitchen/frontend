      ...
      try {
        ...
        if (selectedKitchen !== 'all') {
          const filteredChats = mockChats.filter(
            chat => chat.kitchenId === `kitchen_${selectedKitchen}`
          );
          ...
          <div style={{ display: 'flex', gap: '16px' }}>
            <DropdownWrapper ref={dropdownRef}>
              <DropdownButton onClick={() => setDropdownOpen(!dropdownOpen)}>
                {selectedKitchen === 'all' ? '모든 주방' : kitchenList.find(k => k.kitchenId === parseInt(selectedKitchen))?.kitchenName || '모든 주방'}
                ...
                <DropdownList>
                  ...
                  {kitchenList.map(kitchen => (
                    <DropdownItem
                      ...
                      selected={selectedKitchen === kitchen.kitchenId.toString()}
                      onClick={() => {
                        setSelectedKitchen(kitchen.kitchenId.toString());
                        setDropdownOpen(false);
                      }}
                    >
                      {kitchen.kitchenName}
